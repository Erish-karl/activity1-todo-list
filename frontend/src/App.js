    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-100 via-blue-200 to-blue-300 flex items-center justify-center p-6">
      <div className="bg-white/90 shadow-2xl rounded-3xl w-full max-w-3xl p-10 border border-blue-200 backdrop-blur-md">
        ...
        <div className="flex mb-8">
          <input
            type="text"
            className="flex-grow border border-blue-400 rounded-l-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 text-lg bg-blue-50 placeholder-blue-400"
        ...
        <ul className="space-y-3">
          {tasks.map((task) => (
            <li
              key={task.id}
              className="flex justify-between items-center bg-blue-50 rounded-xl px-5 py-3 shadow-sm hover:shadow-md transition border border-blue-100"
            >
              {editingTask === task.id ? (
                <input
                  type="text"
                  className="flex-grow border border-blue-400 rounded-lg px-3 py-1 mr-2 bg-white"
                  value={updatedTitle}
                  onChange={(e) => setUpdatedTitle(e.target.value)}
                />
              ) : (
                <span className="text-lg text-blue-900 font-medium">{task.title}</span>
              )}
        ...
        {tasks.length === 0 && (
          <p className="text-center text-blue-600 mt-6 text-lg italic">
            No tasks yet...
          </p>
        )}
      </div>
